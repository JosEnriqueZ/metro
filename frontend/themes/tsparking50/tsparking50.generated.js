...
window.Vaadin.Flow['_vaadintheme_tsparking50_globalCss'] = window.Vaadin.Flow['_vaadintheme_tsparking50_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_tsparking50_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_tsparking50_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_tsparking50_globalCss'].push(target);
  }
  if (!document['_vaadintheme_tsparking50_componentCss']) {
    
    document['_vaadintheme_tsparking50_componentCss'] = true;
  }
